...
app.post('/api/providers/:id/validate', async (req, res) => {
    ...
    try {
        const result = await providers.validateKey(id, apiKey);
        res.json(result);
    }
    catch (error) {
...
app.post('/api/providers/:id/save-key', async (req, res) => {
    ...
    const { apiKey } = req.body;
    try {
        const result = await providers.saveKey(id, apiKey);
        res.json(result);
    }
    catch (error) {
        res.json({ valid: false, error: error.message });
...
app.delete('/api/providers/:id/key', (req, res) => {
    const { id } = req.params;
    providers.removeKey(id);
    res.json({ success: true });
});
app.get('/api/providers/:id/models', async (req, res) => {
    const { id } = req.params;
    try {
        const models = await providers.fetchModels(id);
        res.json(models);
    }
    catch (error) {
        res.status(500).json({ error: error.message });
...
app.post('/api/models/active', (req, res) => {
    ...
    if (modelId)
        providers.setActiveModel(modelId);
    res.json({
        success: true,
